...
USER_NAME = os.environ.get('USER_NAME', 'Mohammed-Muhaimin')
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    def format_num(num):
        return f"{'{:,}'.format(num)}" if isinstance(num, int) else str(num)

    repo_data = format_num(repo_data)
    contrib_data = format_num(contrib_data)
    star_data = format_num(star_data)
    commit_data = format_num(commit_data)
    follower_data = format_num(follower_data)
    loc_total = format_num(loc_data[2])
    loc_add = format_num(loc_data[0])
    loc_del = format_num(loc_data[1])
    ...
    MAX_LEN = 60
    repo_dots = 5
    line1_pipe_col = 8 + (repo_dots + 2) + len(repo_data) + len(f" {{Contributed: {contrib_data}}} ")
    line1_val_len = len(f" {repo_data} {{Contributed: {contrib_data}}} | Stars:") + len(star_data)
    star_dots = MAX_LEN - 8 - (repo_dots + 2) - line1_val_len - 2
    if star_dots < 0: star_dots = 0
    ...
    line2_pre_pipe_len = 11 + len(commit_data) + 1
    commit_dots_str_len = line1_pipe_col - line2_pre_pipe_len
    commit_dots = commit_dots_str_len - 2
    if commit_dots < 0: commit_dots = 0
    line2_val_len = len(f" {commit_data} | Followers:") + len(follower_data)
    follower_dots = MAX_LEN - 11 - (commit_dots + 2) - line2_val_len - 2
    if follower_dots < 0: follower_dots = 0
...
if __name__ == '__main__':
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2005, 6, 25))
